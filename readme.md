...
```
echo 'snake_case' | awk -F '_' '{ for(i=1; i<=NF; i++) {printf toupper(substr($i,1,1)) substr($i,2) "\n"}}'
```

##### snake_case to camelCase

```
echo 'snake_case' | awk -F '_' '{ printf $1; for(i=2; i<=NF; i++) {printf toupper(substr($i,1,1)) substr($i,2) "\n"}}'
```
